...
class ThorCamInterface(QtWidgets.QWidget):
	def __init__(self, cameraName="imaging"):
		super().__init__()
		...
		# self.thorcam = ThorCam(cameraName) #Options: "trapping", "imaging", "test"
		# self.thorcam = ThorCam("trapping")
		# self.thorcam = ThorCam("imaging")
		...
		self.numIdentifiedTrapsLabel = QtGui.QLabel("# Identified traps:")
	...
	def prepareForCalibration(self):
		self.settings.setExposure(0.01)
	...
	def updateNumTrapsLabel(self, num):
		self.numIdentifiedTrapsLabel.setText("# Identified Traps: %d" %int(num))

	def updateImage(self, num_traps=1000, ignoreZeroOrder = False):
		...
		traps = self.identifyTraps(im, num_traps=num_traps, threshold=self.settings.getThreshold(),
								   ignoreZeroOrder=ignoreZeroOrder)

		self.updateNumTrapsLabel(len(traps))
		...
		if self.settings.getShowTargets():
			for c in self.slmCoordinates:
				...
				markPosition(rgbImage, pos, radius=2, color=[255, 0, 255])
		...
		hist, bin_edges = np.histogram(traps[:, 2], bins=52, range=[0, 260])
	...
	def identifyTraps(self, image, num_traps=1000, threshold=80, blockade_size=10,
						ignoreZeroOrder=False, zeroOrderBlockade = 20):
...
def gaussian_2D(pos, amplitude, x0, y0, sigma, offset):
	...
	if amplitude < 10.0 or sigma < 3 or sigma > 8:
		return -100000
